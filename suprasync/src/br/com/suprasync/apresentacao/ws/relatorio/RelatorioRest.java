...
public class RelatorioRest {
	...
	@PUT
	@Path("/graficoOcorrencias1")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public String obterOcorrenciasSolucionadasSolucionadasDevGraf(String filtro) throws NamingException {		

		GenericFacade genericFacade = new GenericFacade();
		StringBuilder nativeQuery = new StringBuilder();
		nativeQuery.append("select data as coluna1, sum(desenvolvimento) as coluna2, sum(solucionadas) as coluna3")
		.append(" from")
		.append(" (	select datepart(year, data_cadastro) as ano , substring(convert(varchar, data_cadastro, 3), 4,5) as data,")
		.append(" 1 as desenvolvimento, 0 as solucionadas, func_codigo_solicitacao, clifor_codigo")
		.append(" from sac_ocorrencia where data_cadastro > '01/01/2014'")
		.append(" and sac_ocorrencia.codigo in (select sacocor_codigo from sac_ocorrencia_desenvolvimento sod)")
		.append(" Union ALL")
		...
		for (Iterator<Object> iterator = resultList.iterator(); iterator.hasNext();) {
			...
			jo.addProperty("solucionadasDev", (Integer) result[1]);
